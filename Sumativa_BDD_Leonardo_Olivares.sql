...
BEGIN
    ...
    VALUES (empleado_seq.NEXTVAL, 'Laura González', 'Recursos Humanos', TO_DATE('2012-06-15', 'YYYY-MM-DD'), 2800);
    INSERT INTO Empleados (empleado_id, nombre, departamento, fecha_contratacion, salario) 
    VALUES (empleado_seq.NEXTVAL, 'Andrés Pérez', 'Ventas', TO_DATE('2018-11-23', 'YYYY-MM-DD'), 3200);
    INSERT INTO Empleados (empleado_id, nombre, departamento, fecha_contratacion, salario) 
    VALUES (empleado_seq.NEXTVAL, 'María Torres', 'Tecnología', TO_DATE('2021-02-10', 'YYYY-MM-DD'), 4500);
    INSERT INTO Empleados (empleado_id, nombre, departamento, fecha_contratacion, salario) 
    VALUES (empleado_seq.NEXTVAL, 'Juan Ramírez', 'Marketing', TO_DATE('2015-08-30', 'YYYY-MM-DD'), 3400);
    INSERT INTO Empleados (empleado_id, nombre, departamento, fecha_contratacion, salario) 
    VALUES (empleado_seq.NEXTVAL, 'Sofía López', 'Administración', TO_DATE('2019-05-20', 'YYYY-MM-DD'), 2900);
...
SELECT TO_CHAR(fecha, 'YYYY-MM') AS año_mes, AVG(cantidad) AS promedio_mensual
FROM Ventas
GROUP BY TO_CHAR(fecha, 'YYYY-MM')
ORDER BY promedio_mensual DESC
FETCH FIRST 1 ROW ONLY; --ordena los resultados de mayor a menor y selecciona el periodo con el promedio más alto usando FETCH FIRST 1 ROW ONLY.
...
, ya que no hay clientes registrados en el ultimo año
*/
SELECT COUNT(*) AS ventas_ultimo_año
FROM Ventas
...
/*
--Aca verifique que aunque hayan ventas en el ultimo año, no hay clientes registrados 
 despues de noviembre de 2023, mas bien el ultimo se registró en octubre y por esta razon retornara 0 ca y en la ultima consulta
*/
SELECT COUNT(*) AS clientes_registrados_ultimo_año
FROM Clientes
WHERE fecha_registro >= ADD_MONTHS(SYSDATE, -12); 

/*
USO DE JOIN con la tabla Productos para acceder a los precios de los productos a través de la columna producto_id.
ADEMAS USO OTRO JOIN con la tabla Clientes para acceder a los datos de los clientes a través de la columna cliente_id.
*/
...
/* Aca con MONTHS_BETWEEN(SYSDATE, fecha_contratacion) calcule la diferencia en meses entre la fecha actual y la fecha de contratación.
Y al dividir por 12 y truncar, obtengo la antigüedad en años.
*/
COMMIT;
